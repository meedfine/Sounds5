...
import { app, protocol, BrowserWindow, Tray, screen, ipcMain, Rectangle } from "electron";
...
function createMain() {
  ...
  win.on("will-resize", (event, bounds) => {
    if (bounds.width < 1000 || bounds.height < 660) {
...
function createTray() {
  ...
  if (process.env.WEBPACK_DEV_SERVER_URL) {
    ...
    trayWin.loadURL(process.env.WEBPACK_DEV_SERVER_URL);
  } else {
    createProtocol("app");
    trayWin.loadURL("app://./index.html");
  }
  trayIcon.on("click", () => {
    win.show();
